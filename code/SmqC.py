...
def plots(submission, output, sub):
    ...
    plot_circular_bar_graph(test['correct'].groupby(test['target_congruent']).mean(), 'Target Congruent', 'accuracy_target_congruence')
    sns.color_palette("rocket_r", as_cmap=True)
    sns.boxplot(x='target_congruent', y='response_time', data=test, showfliers=False, color='white')
    sns.stripplot(x='target_congruent', y='response_time', data=test, jitter=True, alpha=0.8, hue='correct')
    plt.savefig(os.path.join(output, f'{sub}_rt_congruence.png'))
